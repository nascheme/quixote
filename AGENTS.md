...
- URL traversal and the `_q_*` protocol (`_q_exports`, `_q_index`, `_q_lookup`,
  `_q_traverse`): `quixote/directory.py` and `doc/public-api.txt` section 2. The
...
- PTL templates (`@ptl_html` / `@ptl_plain` and the `.ptl` import hook):
  `doc/PTL.txt`, `quixote/ptl/`. Call `quixote.enable_ptl()` once at startup,
  before importing any `.ptl` module (and after importing ZODB, if used).
- `htmltext` and the escaping model: `doc/tstring_html.txt`, `quixote/html/`.
...
- Request / response / session accessors (`get_request`, `get_response`,
  `get_session`, `redirect`, ...): `quixote/publish.py`, `doc/session-mgmt.txt`.
- Forms and widgets: `doc/widgets.txt`, `quixote/form/` (`quixote/form1/` is
  legacy).
