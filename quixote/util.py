...
class StaticBundle(Directory):
    ...
    def _get_filename_prefix(self, filename):
        """Return path prefix to use for 'filename'.  It has based on
    ...
    def make_path(self, *filenames):
        ...
        prefix = self._get_filename_prefix(key)
        path = '%s/%s/%s' % (self.basepath, prefix, key)
        self.paths[key] = path
        return path

    def _q_traverse(self, path):
        if len(path) == 1:
            # if prefix component missing, allow direct path, no caching
            filename = path[0]
            prefix = ''
            cache_time = None
        elif len(path) == 2:
            # path with a cache busting integer prefix
            filename = path[1]
            if filename == '..':
